...
def build_optim(args, model, checkpoint):
    ...
    if checkpoint is not None:
        # print('checkpoint_keys:', checkpoint.keys())
...
class Bert(nn.Module):
    def __init__(self, large, temp_dir, finetune=False):
        ...
        self.model.config.bos_token_id = 8002
        self.model.config.eos_token_id = 8003
        self.model.config.vocab_size = 8004
        # else:
    ...
    def forward(self, x, segs, mask):
        ...
        if(self.finetune):
            ...
            # print('output[0]_type:', type(output[0]))
            # top_vec, no_top_vec = self.model(x, token_type_ids=segs, attention_mask=mask)
            top_vec = self.model(x, token_type_ids=segs, attention_mask=mask)[0]
            # top_vec, no_top_vec = self.model(x, token_type_ids=segs, attention_mask=mask)
            # print('top_vec in Bert:',top_vec)
            # print('no_top_vec in Bert:', no_top_vec)
        else:
            self.eval()
            with torch.no_grad():
                top_vec = self.model(x, token_type_ids=segs, attention_mask=mask)[0]
...
class AbsSummarizer(nn.Module):
    def __init__(self, args, device, checkpoint=None, bert_from_extractive=None):
        ...
        self.device = device
        self.bert = Bert(args.large, args.temp_dir, args.finetune_bert)
        # extractive 모델이 있던 경우 해당 모델 dict를 통해 모델 가져오기 
