...
def train_abs_multi(args):
    ...
    mp = torch.multiprocessing.get_context('spawn') # or set_start_method
...
def run(args, device_id, error_queue):
    ...
    setattr(args, 'gpu_ranks', [int(i) for i in args.gpu_ranks])    # args에 'gpu_ranks'를 쭉 뽑아 리스트업함
...
def train_abs_single(args, device_id):
    ...
    from kobert_tokenizer1 import KoBERTTokenizer
    ...
    symbols = {'BOS':tokenizer.bos_token_id, 'EOS': tokenizer.eos_token_id,
               'PAD': tokenizer.pad_token_id, 'EOQ': tokenizer.eos_token_id}
